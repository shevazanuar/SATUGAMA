...
import { AudioSFXPlayer } from "@/components/audio-sfx-player";
import { ModelViewer3D } from "@/components/model-viewer-3d";
import { GlitchPlayground } from "@/components/glitch-playground";
import { FeaturesExpand } from "@/components/features-expand";
  ...
  return (
    <div className="bg-[#090a14] text-slate-100 min-h-screen antialiased">
      ...
      <main>
        ...
        {/* ═══════════════════════════════════════
            INTERACTIVE PREVIEW (per category)
        ═══════════════════════════════════════ */}
        {(product.category === "ui" || product.category === "3d") && (
          <section className="py-20 border-t border-white/[0.04]">
            <div className="max-w-6xl mx-auto px-5">
              <div className="text-center mb-12">
                <p className={`section-label ${c.label}`}>Demo Interaktif</p>
                <h2
                  className="text-3xl md:text-4xl font-bold text-white"
                  style={{ fontFamily: "var(--font-sora)" }}
                >
                  {product.category === "ui" ? "Coba Langsung Asetnya" : "Preview Model 3D"}
                </h2>
                <p className="text-slate-500 text-base mt-3 max-w-lg mx-auto">
                  {product.category === "ui"
                    ? "Dengarkan efek suara dan eksplorasi komponen UI secara langsung di browser."
                    : "Putar dan eksplorasi model 3D low-poly secara real-time. Drag untuk memutar, klik wireframe untuk melihat mesh."}
                </p>
              </div>
              <div className="space-y-6">
                ...
                {product.category === "3d" && <ModelViewer3D />}
              </div>
            </div>
          </section>
        )}
          ...
          <div className="max-w-6xl mx-auto px-5">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
              ...
              <div className="space-y-4 lg:pt-16">
                ...
                <a
                  href={`https://wa.me/62882005486575?text=Halo%20SatuGama!%20%F0%9F%91%8B%20Saya%20tertarik%20dengan%20*${encodeURIComponent(product.title)}*%20(${encodeURIComponent(product.price)}).%20Bisa%20info%20cara%20pembeliannya%3F%20Terima%20kasih!%20%F0%9F%8E%AE`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full flex items-center justify-center gap-2.5 py-3.5 btn-primary text-white font-bold text-sm rounded-2xl hover:scale-[1.02] transition-transform"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="h-4 w-4 fill-current shrink-0" aria-hidden="true"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
          ...
          <div className="max-w-6xl mx-auto px-5">
            <div className="relative rounded-3xl border border-indigo-500/20 bg-indigo-500/5 px-8 py-16 text-center overflow-hidden">
              ...
              <div className="relative z-10 space-y-5 max-w-lg mx-auto">
                <p className={`section-label ${c.label}`}>Siap Mulai?</p>
                <h2
                  className="text-3xl md:text-4xl font-bold text-white"
                  style={{ fontFamily: "var(--font-sora)" }}
                >
                  Dapatkan <span className="text-gradient">{product.title}</span>
                </h2>
                <p className="text-slate-400 leading-relaxed text-sm">
                  Pesan sekarang secara instan via WhatsApp untuk mendapatkan file aset beserta lisensi komersial penuh.
                </p>
                <div className="flex justify-center pt-2">
                  <a
                    href={`https://wa.me/62882005486575?text=Halo%20SatuGama!%20%F0%9F%91%8B%20Saya%20tertarik%20dengan%20produk%20*${encodeURIComponent(product.title)}*%20(${encodeURIComponent(product.price)}).%20Bagaimana%20cara%20pembeliannya%3F%20%F0%9F%8E%AE`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center justify-center gap-2.5 px-8 py-4 btn-primary text-white font-bold rounded-2xl text-sm hover:scale-[1.02] transition-transform"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="h-5 w-5 fill-current shrink-0" aria-hidden="true"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                    Hubungi Kami via WhatsApp
                  </a>
                </div>
                <p className="text-slate-600 text-xs">
                  Respon instan · Lisensi komersial inklusif · Harga {product.price}
                </p>
              </div>
            </div>
          </div>
